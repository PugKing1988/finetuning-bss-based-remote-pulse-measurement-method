...
/* Prints a stack of K matrices of siz
e NxN */
...
void md(double *vec, int T)
{
	...
	for (i=0;i<T/2;i++)
	{
	double noise=0;//0.5 * ( (double) rand() / (double) RAND_MAX ) - 1.0;

		vec[i] =  noise+10*sin(i/10.0);//2.0 * ( (double) rand() / (double) RAND_MAX ) - 1.0  ; 
//			vec[i] =  10;
		y=0;
	}
	...
	for (i=T/2;i<T;i++)
	{
		...
		vec[i]=v+noise;
		if (i%10==0)
			v=10 * ( (double) rand() / (double) RAND_MAX ) - 5.0;
//		vec[i] =(i%10)+noise;
	}
